...
本项目旨在系统评估不同 DNA tokenization 策略在物种分类任务中的性能，重点关注策略与序列长度的交互效应。
...
本项目手动实现了三种 tokenization 策略。

### 重叠 k-mer

使用固定长度的滑动窗口遍历序列，步长（stride）设为 1。

- **参数**：k = 3, 4, 5, 6（主实验固定 k=5）

### Canonical k-mer

在重叠 k-mer 基础上，对每个 k-mer 及其反向互补取字典序较小者，将 DNA 双链信息合并。

- **参数**：k = 5（主实验）

### BPE (Byte Pair Encoding)

基于训练集学习合并规则，通过迭代合并频次最高的相邻 token 对来构建词表。

- **参数**：词表大小 = 200, 500, 1000（主实验用 500）
## 模型架构
...
| Canonical k-mer | 短 reads | 512 | 320.7 | 0.69 |
| BPE | 短 reads | 500 | 191.6 | 23.85 |
| 重叠 k-mer | 长 contigs | 1024 | 2996.0 | 0.68 |
| Canonical k-mer | 长 contigs | 512 | 2996.0 | 5.78 |
| BPE | 长 contigs | 500 | 1792.5 | 234.35 |
...
| BPE (vocab=500) | 长 contigs | 84.21% | 0.73 |

## 主要发现

- **k-mer 方法准确率最高**：Baseline 下达到 92.5%，优于 BPE（78.1%-78.9%）
- **BPE + CNN 提升显著**：从 78.1% 提升到 93.15%，但推理时间增加（680 ms）
- **Canonical k-mer 词表减半**：从 1024 降至 512，准确率不变
- **长序列更易分类**：所有策略在长序列上准确率更高
- **层级分类准确率 93.10%**（长序列，加分任务）

 ## 加分任务：层级分类

本项目还探索了层级分类（从门到种的 taxonomy 路径预测），使用层级 CNN 同时预测门和种。在 4 个物种（2 个门）的数据集上，结果如下：

| 长度 | 门准确率 | 种准确率 | 层级准确率 |
|------|---------|---------|-----------|
| 短 reads | 93.36% | 86.05% | 85.05% |
| 长 contigs | 100.00% | 93.10% | 93.10% |

模型在长片段上实现了 100% 的门预测准确率和 93.10% 的层级准确率，表明模型能有效学习物种的层级分类关系。
## 作者
姓名：[Cailing Chen]
